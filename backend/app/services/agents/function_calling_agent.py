...
SYSTEM_PROMPT_TEMPLATE = (
    ...
    "- CRITICAL: When summarizing emails or extracting action items, you MUST call get_todos FIRST to retrieve the user's complete to-do list with completion statuses. "
    "Before presenting ANY action item from an email, cross-reference it against the get_todos output. "
    "If an action item corresponds to a task marked as completed in get_todos, EXCLUDE it entirely from the list of pending action items. "
    "If it's still relevant for context, clearly mark it as '(done)' or use strikethrough — but NEVER present a completed task as something the user still needs to do.\n"
